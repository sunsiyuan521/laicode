...
public class KthClosestPointTo000 {
    public List<Integer> closest(int[] a, int[] b, int[] c, int k) {
        ...
        while(k > 0) {
            cur = minHeap.poll();
            List<Integer> curPoint = Arrays.asList(cur.get(0) + 1, cur.get(1), cur.get(2));
            if(curPoint.get(0) < a.length && visited.add(curPoint)){
                minHeap.offer(curPoint);
            }
            curPoint = Arrays.asList(cur.get(0), cur.get(1) + 1, cur.get(2));
            if(curPoint.get(1) < b.length && visited.add(curPoint)){
                minHeap.offer(curPoint);
            }
            curPoint = Arrays.asList(cur.get(0), cur.get(1), cur.get(2) + 1);
            if(curPoint.get(2) < c.length && visited.add(curPoint)){
                minHeap.offer(curPoint);
            }
            k--;
        }
